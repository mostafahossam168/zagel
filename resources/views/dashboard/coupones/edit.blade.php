@extends('dashboard.layouts.backend', ['title' => 'تعديل عرض'])
...
@section('contant')
    <div class="main-side">
        ...
        <form action="{{ route('dashboard.coupones.update', $item->id) }}" method="post">
            @csrf
            @method('PUT')
            <div class="row g-4">
                <div class="col-12 col-md-4 col-lg-3">
                    <label class="special-input">
                        <span>الكود</span>
                        <div class="box-input">
                            <input type="text" name="code" value="{{ $item->code }}" id="numberonly">
                        </div>
                ...
                <div class="col-12 col-md-4 col-lg-3">
                    ...
                    <select name="course_id" id="tax" class="form-select select-setting">
                        ...
                        @foreach ($courses as $course)
                            <option value="{{ $course->id }}" @selected($course->id == $item->course_id)>{{ $course->title }}</option>
                        @endforeach
                    </select>
                    @error('course_id')
                        <span class="text-danger">{{ $message }}</span>
                ...
                <div class="col-12 col-md-4 col-lg-3">
                    ...
                    <select name="discount_type" id="" class="form-select select-setting">
                        ...
                        @foreach (\App\enums\DiscountTypeCoupone::cases() as $type)
                            <option value="{{ $type }}" @selected($item->discount_type == $type)>
                                {{ $type->name() }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-12 col-md-4 col-lg-3">
                    <label class="special-input">
                        ...
                        <div class="box-input">
                            <input type="number" min="1" name="usage_limit" value="{{ $item->usage_limit ?? 1 }}"
                                id="">
                        </div>
                        @error('usage_limit')
                            <span class="text-danger">{{ $message }}</span>
                ...
                <div class="col-12 col-md-4 col-lg-3">
                    <label class="special-input">
                        <span>البدايه</span>
                        <div class="box-input">
                            <input type="datetime-local" min="1" name="start_date" value="{{ $item->start_date }}"
                                id="">
                        </div>
                        @error('start_date')
                            <span class="text-danger">{{ $message }}</span>
                ...
                <div class="col-12 col-md-4 col-lg-3">
                    <label class="special-input">
                        <span>النهايه</span>
                        <div class="box-input">
                            <input type="datetime-local" min="1" name="end_date" value="{{ $item->end_date }}"
                                id="">
                        </div>
                        @error('end_date')
                            <span class="text-danger">{{ $message }}</span>
